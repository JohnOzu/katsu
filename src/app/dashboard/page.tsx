...
export default function DashboardPage() {
	...
	return (
		<div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex">
			...
			{/* ── Sidebar ── */}
			<aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col h-screen sticky top-0">
				...
				{!db.selectedClass ? (
					<div className="flex flex-col items-center justify-center min-h-screen text-center px-6">
						...
						<div className="flex items-center space-x-3">
							...
							<button onClick={db.openJoinModal} className="px-6 py-3 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-2xl font-semibold hover:border-green-400 hover:shadow-lg transition-all transform hover:-translate-y-0.5 cursor-pointer">
								Join a Class
							</button>
						</div>
					</div>
					...
					<div className="max-w-5xl mx-auto px-6 py-8">
						...
						<div className="mb-8">
							<div className="flex items-center space-x-3 mb-1">
								<h1 className="text-3xl font-black text-slate-900 dark:text-white">{db.selectedClass.name}</h1>
						...
						{/* Tabs */}
						<div className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl mb-6 w-fit">
							{(['tasks', 'subjects', 'members', 'calendar'] as const).map((tab) => (
								<button key={tab} onClick={() => db.setActiveTab(tab)} className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all capitalize cursor-pointer ${db.activeTab === tab ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
									...
									<div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-4 mb-4 space-y-4">
										<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
											...
											<div className="space-y-1.5">
												...
												<div className="flex flex-col space-y-1">
													<button onClick={() => setSubjectFilter('all')} className={`text-left px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${subjectFilter === 'all' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
														All subjects
													</button>
													<button onClick={() => setSubjectFilter('none')} className={`text-left px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${subjectFilter === 'none' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
														No subject
													</button>
													{db.subjects.map(s => (
														<button key={s.id} onClick={() => setSubjectFilter(s.id)} className={`text-left px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer truncate ${subjectFilter === s.id ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
															{s.name}
														</button>
													))}
												</div>
											...
											<div className="space-y-1.5">
												...
												<div className="flex flex-col space-y-1">
													{([
														...
														<button key={val} onClick={() => setDeadlineFilter(val)} className={`text-left px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${deadlineFilter === val ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
															{label}
														</button>
													))}
												</div>
											...
											<div className="space-y-1.5">
												...
												<div className="flex flex-col space-y-1">
													{([
														...
														<button key={val} onClick={() => setSortOrder(val)} className={`text-left px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${sortOrder === val ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
															{label}
														</button>
													))}
												</div>
												...
												>
													<div className="flex items-start p-4 space-x-4">
														<button
															onClick={(e) => {
																e.stopPropagation();
																db.toggleCompletion(task, myCompletion?.isPublic ?? true)
															}}
														...
														<div className="hidden group-hover:flex items-center space-x-1">
															...
															{db.selectedClass && canDelete(db.selectedClass?.myRole) && (
																<button 
																	...
																	className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-500 transition-colors cursor-pointer"
																>
																	<svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
																</button>
															)}
						...
						{db.activeTab === 'members' && (
							<div className="space-y-6">
										...
										{db.invites.length === 0 ? (
											...
											<div className="space-y-2">
												{db.invites.map((invite) => (
													<div key={invite.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-700">
								...
								<div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden">
									<div className="grid grid-cols-12 px-5 py-2.5 bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700 text-xs font-semibold text-slate-400 uppercase tracking-wider">
									...
									{db.members.map((m) => {
										...
										return (
											<div key={m.userId} className="grid grid-cols-12 items-center px-5 py-3.5 border-b border-slate-50 dark:border-slate-700/50 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
												...
												<div className="col-span-3 flex items-center justify-end space-x-2">
													{db.selectedClass && db.selectedClass.myRole === 'owner' && !isMe && (
														<>
															<select
																value={m.role}
																onChange={(e) => db.updateMemberRole(m.userId, e.target.value as Role)}
																className="text-xs px-2.5 py-1.5 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all cursor-pointer bg-white dark:bg-slate-700"
															>
						...
						{db.activeTab === 'calendar' && (() => {
							...
							// Tasks that have a deadline this month
								...
								<div className="flex gap-6">
									...
									<div className="flex-1 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 overflow-hidden">
										...
										<div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
											<button
												onClick={() => { setCalendarDate(new Date(year, month - 1, 1)); setSelectedCalendarDay(null); }}
										...
										<div className="grid grid-cols-7">
											...
											{Array.from({ length: firstDay }).map((_, i) => (
												<div key={`empty-${i}`} className="min-h-[80px] border-b border-r border-slate-50 dark:border-slate-700/50" />
											))}
											...
											{Array.from({ length: daysInMonth }).map((_, i) => {
												...
												return (
													<div
														...
														className={`min-h-[80px] p-2 border-b border-slate-50 dark:border-slate-700/50 cursor-pointer transition-colors ${!isLastCol ? 'border-r' : ''} ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/30'}`}
													>
														...
														<div className="space-y-0.5">
															{dayTasks.slice(0, 3).map(task => {
																...
																return (
																	<div key={task.id} className={`flex items-center space-x-1 px-1.5 py-0.5 rounded-md ${isDone ? 'bg-green-50 dark:bg-green-900/20' : 'bg-slate-100 dark:bg-slate-700'}`}>
																		<div className={`w-1.5 h-1.5 rounded-full shrink-0 ${isDone ? 'bg-green-500' : dotColor(task.deadline!)}`} />
																		<span className={`text-[10px] font-medium truncate ${isDone ? 'text-green-600 dark:text-green-400 line-through' : 'text-slate-600 dark:text-slate-300'}`}>
																			{task.name}
																		</span>
																	</div>
																);
										...
										{/* Legend */}
										<div className="flex items-center space-x-4 px-6 py-3 border-t border-slate-100 dark:border-slate-700">
											<span className="text-xs text-slate-400 font-medium">Deadline urgency:</span>
											{[
												{ color: 'bg-red-500',    label: 'Overdue' },
												{ color: 'bg-orange-500', label: 'Today' },
												{ color: 'bg-yellow-500', label: '≤ 3 days' },
												{ color: 'bg-blue-500',   label: '≤ 1 week' },
												{ color: 'bg-green-500',  label: 'Later' },
											].map(({ color, label }) => (
															...
															<div key={task.id} className={`p-3 rounded-2xl border transition-all ${isDone ? 'bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800' : 'bg-slate-50 dark:bg-slate-700/50 border-slate-100 dark:border-slate-700'}`}>
																<div className="flex items-start space-x-2.5">
																	<button
																		...
																		className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all cursor-pointer ${isDone ? 'bg-green-500 border-green-500' : 'border-slate-300 dark:border-slate-500 hover:border-green-400'}`}
																	>
																	...
																	<div className="flex-1 min-w-0">
																		<p className={`text-sm font-semibold ${isDone ? 'line-through text-slate-400' : 'text-slate-900 dark:text-white'}`}>
																			{task.name}
																		</p>
																		{task.description && (
																		...
																		<div className="flex items-center flex-wrap gap-1.5 mt-1.5">
																			...
																			{subject && (
																				<span className="text-xs px-1.5 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 rounded-md font-medium border border-indigo-100 dark:border-indigo-800">
																					{subject.name}
																				</span>
																			)}
																		</div>
													...
													<div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-700 text-left space-y-1.5">
														...
														{Object.entries(tasksByDay)
															...
															.map(([day, tasks]) => (
																<button
																	key={day}
																	onClick={() => setSelectedCalendarDay(new Date(year, month, Number(day)))}
																	className="w-full flex items-center justify-between px-3 py-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer group"
																>
				...
				<Modal title={db.editingTask ? 'Edit Task' : 'Add Task'} onClose={db.closeTaskModal}>
					<div className="space-y-4">
						...
						<div className="space-y-1.5">
							...
							>
								<MDEditor
									value={db.taskForm.description}
									onChange={(val) => db.setTaskForm(f => ({ ...f, description: val ?? '' }))}
									preview="edit"
									height={200}
						...
						<div className="space-y-4">
							<div className="space-y-3">
								...
								<div className="grid grid-cols-2 gap-2">
									<button onClick={() => db.setInviteForm(f => ({ ...f, useGenerated: true }))} className={`p-3 rounded-xl border-2 text-sm font-medium transition-all cursor-pointer ${db.inviteForm.useGenerated ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' : 'border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-500'}`}>
										🎲 Auto-generate
									</button>
			...
			{viewingTask && (() => {
				...
				return (
					...
					>
						<div
							className="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden animate-fadeInUp"
						...
						>
							...
							<div className="flex items-start justify-between px-6 py-5 border-b border-slate-100 dark:border-slate-700">
								<div className="flex items-start space-x-3 flex-1 min-w-0">
									<button
										...
										className={`mt-1 w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all cursor-pointer ${isDone ? 'bg-green-500 border-green-500' : 'border-slate-300 dark:border-slate-500 hover:border-green-400'}`}
									>
										{isDone && <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
									</button>
									<div className="flex-1 min-w-0">
										<h2 className={`text-xl font-black ${isDone ? 'line-through text-slate-400' : 'text-slate-900 dark:text-white'}`}>
											{task.name}
										</h2>
										<div className="flex items-center flex-wrap gap-2 mt-2">
											{subject && (
												<span className="text-xs px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 rounded-md font-medium border border-indigo-100 dark:border-indigo-800">
													{subject.name}
												</span>
											)}
											{totalCompletions > 0 && (
												<span className="text-xs px-2 py-0.5 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-300 rounded-md font-medium border border-green-100 dark:border-green-800">
													✓ {totalCompletions} done
												</span>
											)}
											{isDone && (
												<span className="text-xs px-2 py-0.5 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded-md font-semibold">
													You completed this
												</span>
											)}
										</div>
								...
								<div className="flex items-center space-x-1 ml-3 shrink-0">
									{canUpdate(db.selectedClass!.myRole) && (
										<button
											onClick={() => { setViewingTask(null); db.openEditTask(task); }}
								...
								{task.description ? (
									<div data-color-mode="auto">
										<div className="prose prose-sm dark:prose-invert max-w-none
											...
											prose-code:bg-slate-100 dark:prose-code:bg-slate-700 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-sm
											prose-ul:text-slate-600 dark:prose-ul:text-slate-300
											prose-ol:text-slate-600 dark:prose-ol:text-slate-300
											prose-blockquote:border-blue-400 prose-blockquote:text-slate-500 dark:prose-blockquote:text-slate-400
							...
							{publicCompletions.length > 0 && (
								<div className="px-6 pb-5">
									...
									<div className="flex items-center space-x-1.5">
										{publicCompletions.slice(0, 8).map(c => (
											<Avatar key={c.userId} profile={db.memberProfiles[c.userId]} size={28} />
										))}
										{publicCompletions.length > 8 && (
											<span className="text-xs text-slate-400 ml-1">+{publicCompletions.length - 8} more</span>
										)}
									</div>
								</div>
							)}
							...
							{/* Footer */}
							{isDone && (
								<div className="px-6 pb-5 border-t border-slate-50 dark:border-slate-700 pt-4">
