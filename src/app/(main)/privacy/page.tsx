			...
			<div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
				...
				<div className="space-y-4 sm:space-y-6">
					{sections.map((section, i) => (
						<div key={section.title} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow p-5 sm:p-7">
							<div className="flex items-start space-x-4">
								<div className="w-8 h-8 bg-linear-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">{i + 1}</div>
								<div>
									<h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mb-2">{section.title}</h2>
				...
				<div className="mt-8 sm:mt-10 bg-linear-to-br from-blue-600 to-indigo-700 rounded-2xl p-7 sm:p-8 text-white text-center">
					...
					<Link href="/contact" className="inline-block px-6 py-2.5 bg-white text-blue-600 rounded-xl font-semibold text-sm hover:shadow-lg transition-all transform hover:-translate-y-0.5">Contact Us</Link>
				</div>
			</div>
		</div>
	);
