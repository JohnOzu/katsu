...
function JoinPageContent() {
	...
	}, [hydrated, user]); // eslint-disable-line
	...
	return (
		<div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900 flex flex-col">
			<div className="fixed inset-0 overflow-hidden pointer-events-none">
				<div className="absolute top-20 right-20 w-96 h-96 bg-blue-200 dark:bg-blue-900 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse" />
				<div className="absolute top-40 left-20 w-96 h-96 bg-purple-200 dark:bg-purple-900 rounded-full mix-blend-multiply filter blur-3xl opacity-20" />
				<div className="absolute bottom-20 left-1/2 w-96 h-96 bg-pink-200 dark:bg-pink-900 rounded-full mix-blend-multiply filter blur-3xl opacity-30" />
			</div>
			...
			<div className="relative z-10 flex-1 flex items-center justify-center px-4 sm:px-6 py-8">
				<div className="w-full max-w-md animate-fadeInUp">
					<div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-3xl shadow-2xl shadow-blue-500/10 border border-slate-100 dark:border-slate-700 p-7 sm:p-8">

						<div className="text-center mb-7 sm:mb-8">
							<div className="w-12 h-12 sm:w-14 sm:h-14 bg-linear-to-br from-green-400 to-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-green-500/30">
								<svg className="w-6 h-6 sm:w-7 sm:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
								</svg>
							</div>
							<h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">Join a Class</h1>
						...
						{/* Step indicator */}
						<div className="flex items-center justify-center space-x-2 mb-6 sm:mb-7">
							{['code', 'password'].map((s, i) => (
								<div key={s} className="flex items-center space-x-2">
									...
									{i < 1 && <div className={`w-8 h-0.5 ${i < (['code', 'password'].indexOf(step)) ? 'bg-green-400' : 'bg-slate-200 dark:bg-slate-600'}`} />}
								</div>
							))}
						</div>
							...
							<div className="space-y-4">
								<div className="space-y-1.5">
									...
									<input
										...
										maxLength={12}
										className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all font-mono text-center text-lg tracking-widest uppercase"
									/>
								</div>
								<button
									onClick={() => lookupCode()}
						...
						{step === 'password' && classPreview && (
							<div className="space-y-4">
								<div className="p-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-2xl text-center">
									<p className="text-xs text-blue-500 dark:text-blue-400 font-semibold uppercase tracking-wide mb-1">Joining</p>
									<p className="text-lg sm:text-xl font-black text-blue-700 dark:text-blue-300">{classPreview.name}</p>
								</div>

								<div className="space-y-1.5">
									<label className="text-sm font-medium text-slate-700 dark:text-slate-300">Class Password</label>
									<input
										type="password"
										value={password}
										...
										className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
									/>
								</div>
								...
								<button onClick={() => { setStep('code'); setClassPreview(null); setPassword(''); }} className="w-full py-2 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors cursor-pointer">
					...
					<div className="text-center mt-5">
						<Link href="/dashboard" className="text-sm text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors inline-flex items-center space-x-1">
