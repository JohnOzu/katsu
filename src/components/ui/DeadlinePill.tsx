...
export default function DeadlinePill({ deadline }: { deadline: string | null }) {
    const [countdown, setCountdown] = useState(() =>
        deadline ? getCountdown(new Date(deadline)) : ''
    );

    useEffect(() => {
        if (!deadline) return;
        const d = new Date(deadline);
        const id = setInterval(() => setCountdown(getCountdown(d)), 1000);
    ...
    return (
        <div className={`inline-flex flex-wrap items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border w-fit ${dl?.color ?? 'text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600'}`}>
